...
function KanbanColumn(props: KanbanColumnProps) {
  ...
  const composedRef = useComposedRefs((node: HTMLDivElement) => {
    if (disabled) return;
    setNodeRef(node);
  });
    ...
    <KanbanColumnContext.Provider value={columnContext}>
      ...
      >
        <ColumnPrimitive
          ...
          style={composedStyle}
          className={cn(
            'flex size-full flex-col gap-2 rounded-lg border bg-card p-2.5 aria-disabled:pointer-events-none aria-disabled:opacity-50',
            {
...
function KanbanColumnHandle(props: KanbanColumnHandleProps) {
  ...
  const composedRef = useComposedRefs((node: HTMLButtonElement) => {
